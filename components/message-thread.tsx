...
import { CheckCircle2, Clock, XCircle, Phone, DollarSign, Calendar, User, Loader2, ChevronDown, ChevronUp, Undo } from "lucide-react"
...
export function MessageThread({ messages, sender, onUpdateStatus, isLoading, isUpdating, hasNextPage, isLoadingMore, onLoadMore, currentPage, isWaveMode, newMessageIds = new Set(), onMarkAsRead }: MessageThreadProps) {
  ...
  const handleStatusChange = (uid: string, status: "approved" | "no_order" | "refunded") => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "refunded":
        return <Undo className="h-4 w-4 text-blue-600" />
      default:
  ...
  const getStatusBadge = (status: string, statusDisplay: string) => {
    switch (status) {
      ...
      case "refunded":
        return <Badge variant="secondary" className="bg-blue-100 text-blue-800 hover:bg-blue-100">{statusDisplay}</Badge>
      default:
                  ...
                  <div className="flex flex-col sm:flex-row justify-end items-end sm:items-center gap-1 sm:gap-2 mt-3 sm:mt-4">
                    ...
                    >
                      ...
                      <span className={`text-xs font-medium ${
                        ...
                        message.status === "refunded" ? "text-blue-600" :
                        "text-yellow-600"
